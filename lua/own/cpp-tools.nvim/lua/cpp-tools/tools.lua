function mth_to_str(meth)
    disable_class = disable_class or false
    local full = ""
    if meth["type"] ~= "" then
        full = meth["type"] .. " "
    end
    if meth["namespace"] ~= "" then
...
local function get_namespace_list(namespace_str, node, buf)
...
function parse_meth(node, buffer, default_class)
