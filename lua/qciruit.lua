...
M.save_circuit_png = function()
    ...
    -- wrap in standalone LaTeX document with white background and larger margin
    local tex_content = [[
\documentclass[border=8mm, varwidth]{standalone}  % bigger border
\usepackage{qcircuit}
\usepackage{xcolor}
...
\end{document}
    ...
    os.execute(string.format(
        "convert -density 300 %s -quality 100 %s",
        pdf_file, png_file
    ))
