return {
	s("cppmake", {
		...
		i(1, "g++"),
		t({ "", "CFLAGS= " }),
		i(2, "-Wall -Wextra -Werror -pedantic -std=c++20 -Wold-style-cast"),
		t({ "", "LDFLAGS= " }),
		i(3, ""),
		t({ "", "OUT= " }),
		i(4, "out"),
		t({ "", "SRCS = $(shell find $(SRC_DIR) -name '*.cc' -a ! -path '" }),
		i(5, "*/tests/*"),
		t({ "')", "" }),
		t({ "OBJS = $(addsuffix .o, $(basename $(notdir $(SRCS))))", "", "" }),
		i(0),
		t({ "", "", "all: $(OBJS)", "	$(CC) $(LDFLAGS) -o $(OUT) $(OBJS)" }),
